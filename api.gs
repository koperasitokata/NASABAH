...
/**
 * API KOPERASI MANDIRI - STABLE VERSION 3.4
 */
...
function getDashboardData(payload) {
  ...
  if (role === "ADMIN") {
    ...
    results.jadwal_global = getSheetData(ss.getSheetByName("PINJAMAN_AKTIF")).filter(p => p.status === "Aktif");
    ...
    results.penagihan_list = getSheetData(ss.getSheetByName("PINJAMAN_AKTIF")).filter(p => p.status === "Aktif");
...
function cairkanPinjaman(payload) {
  ...
  if (pData) {
    ...
    // LOGIKA BUNGA OTOMATIS BERDASARKAN NOMINAL
    let bungaPersen = 20;
    ...
    // Potongan admin 5% tetap
    if (pemasukanSheet) pemasukanSheet.appendRow([new Date(), pData.id_nasabah, "Admin Cair 5%", amount * 0.05, payload.petugas]);
    
    // Simpanan wajib 5% jika dicentang
    ...
    aktifSheet.appendRow([id, new Date(), pData.id_nasabah, pData.nama, amount, bungaPersen, totalHutang, pData.tenor, cicilan, totalHutang, "Aktif", payload.petugas, new Date(), payload.fotoBukti, id]);
...
function updateNasabah(payload) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idCol]) === String(payload.old_id)) {
      sheet.getRange(i+1, idCol+1, 1, 3).setValues([[payload.id_nasabah, payload.nik, payload.nama]]); // Example update
      return createResponse({ success: true });
    }
  }
  return createResponse({ success: false });
...
function ambilTransport(payload) {
  ...
  // payload.fotoBukti disimpan di kolom ke-6 (bukti cair)
